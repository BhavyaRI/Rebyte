...
export default function LinkList({ myLinks, loading, API_BASE_URL,handleDelete ,onShowQr}) {
  return (
    <div className="flex-1 min-h-[100px] bg-white rounded-xl p-8 border border-gray-300 min-w-0">
      ...
      {myLinks.length === 0 && !loading ? (
        ...
        <div className="flex flex-col gap-4">
          {myLinks.map((link) => (
            <LinkCard key={link._id} link={link} API_BASE_URL={API_BASE_URL} handleDelete={handleDelete} onShowQr={onShowQr} />
          ))}
        </div>
      )}
    </div>
